...
# Layout (relative to project root, which is `causal-final/`):
#   session_data/<sid>.json   ← canonical per-session JSON, tracked in git
#   sessions_csv/<sid>.csv    ← 25-row CSV, one per session, tracked in git
#   data/all_sessions.csv     ← merged CSV (gitignored; regenerated by scripts/merge_data.py)
# Per-session files are uniquely named so multi-contributor pushes never
# conflict; the merged CSV is local-only.
_PROJECT_ROOT = Path(__file__).resolve().parent.parent
DATA_DIR = _PROJECT_ROOT / "data"
SESSIONS_DIR = _PROJECT_ROOT / "session_data"
PER_SESSION_CSV_DIR = _PROJECT_ROOT / "sessions_csv"
...
def _ensure_dirs() -> None:
    ...
    PER_SESSION_CSV_DIR.mkdir(parents=True, exist_ok=True)
...
def write_per_session_csv(session_dict: Dict[str, Any]) -> Path:
    """Write/overwrite a single-session CSV at sessions_csv/<sid>.csv.

    Files are uniquely named by session_id so multi-contributor pushes never
    collide. The full 26-column header is written to every file so each is
    self-contained for analysis. Atomic write to .tmp + rename.
    """
    _ensure_dirs()
    sid = session_dict["session_id"]
    rows = [_row_for_trade(session_dict, t) for t in session_dict["trades"]]
    target = PER_SESSION_CSV_DIR / f"{sid}.csv"
    ...
    for row in rows:
        writer.writerow(row)
    _atomic_write(target, buf.getvalue())
    return target
...
def append_session_to_csv(session_dict: Dict[str, Any]) -> None:
    ...
    Also writes the standalone per-session CSV at sessions_csv/<sid>.csv so
    each session has a tracked, conflict-free file in git.
    """
    _ensure_dirs()
    rows = [_row_for_trade(session_dict, t) for t in session_dict["trades"]]
    with _csv_lock():
        ...
        with CSV_PATH.open("a", newline="", encoding="utf-8") as f:
            ...
            for row in rows:
                writer.writerow(row)
    write_per_session_csv(session_dict)


def rewrite_session_rows_in_csv(session_dict: Dict[str, Any]) -> None:
    """Replace all rows for `session_id` in all_sessions.csv AND rewrite the
    per-session CSV at sessions_csv/<sid>.csv (used after survey to
    populate covariate columns)."""
    _ensure_dirs()
    write_per_session_csv(session_dict)
    sid = session_dict["session_id"]
    with _csv_lock():
